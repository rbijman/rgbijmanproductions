...
..\SQL\
	...
	database_config.ini
..\ProcessedData\
	traffic_data.pickles
	weather_data.pickles
...
1) Populate PostgreSQL database and/or pickles files
2) Launch Flask Restfull api to obtain data from PostrgreSQL: python ..\API\api_to_sql_database.py
3) Launch AIweerfiledashboard: python ..\AIWeerfiledashboard\AIweerfiledashboard.py


Code summary:

AIWeerfiledashboard\AIweerfiledashboard.py

The main file that initializes the dashboard and renders all the interactions on the dashboard. The dashboard is build using Dash and consists of a frontend part (app_layout.py) and a backend part (callbacks.py) which are developed as two separate functions to separate the front-end and back-end as much as possible. Both app_layout and callbacks have a 'main()' memberfunctions that serve as initializer. (future idea is to make it classes with init functions that take this role)


AIWeerfiledashboard\Subfunctions\app_layout.py

In the main() of app_layout.py the developer can toggle on/off dashboard tabs to be visualized by calling the subsequent memberfunctions which are defined below the main() member function. Each tab is defined as a member function of the app_layout (likewise in the callbacks function, as explained below). For each tab, the dependency of app_layout on the actual data (i.e. dropdown options/radiobuttons) has been transferred to the backend counterpart (callbacks.py). The dropdowns/radiobuttons that are data dependend are left empty on initializiation and only filled when running the respective callbacks (callbacks.py). De dropdowns/radiobuttons that are independend of the data (e.g. select_traffic_information_type, select_traffic_statistic, select_weather_type) are filled with the respective options directly on instantiation of the layout.


AIWeerfiledashboard\Subfunctions\callbacks.py

In the main() of callbacks.py the developer can likewise as in app_layout.py toggle on/off dashboard tab callbacks, by calling the subsequent memberfunctions which are defined below the main() member function. For each dashboard tab a memberfunction existst which does  the following actions (except for the 'update data' tab, which has a different behavior): 

1) Get dropdown options --> checks if the dropdownoptions are already retrieved from the data and stored. If not, it retrieves and stores. 
2) Populate dropdown menus --> reads the data from the store and populates the datadependend dropdown/radiobutton options
3) Populate the date_range --> checks the year/month selected and populates the date range accordingly
4) Prep the inputs --> Replaces 'All' with the actual options
5) Build the query --> based on all the selected dropdown/radiobutton settings
6) apply the query --> to either the SQL database (using the API) or to the pandas dataframe from pickles directly
7) update the plots
...
AIWeerfiledashboard\Subfunctions\AIweerfile_functions.py
...
Sandboxing\AIweerfile_main.py --> INDEPENDENT FROM THE DASHBOARD!
Sandboxing environment to perform data pre-processing and storage in pickles and PostgreSQL database. Also there are several trials on plotting and analyses.