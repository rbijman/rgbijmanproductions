...
def collect_and_clean_AIweerfiledata(start_date,end_date,ampm_border,shortlong_border):
    ...
    traffic_data[['NLSitNummer','Road','direction','Oorzaak_1','Oorzaak_2','Oorzaak_3','Oorzaak_4','traject']] = data[["NLSitNummer","RouteOms","hectometreringsrichting",'Oorzaak_1','Oorzaak_2','Oorzaak_3','Oorzaak_4','TRAJECTVILD']]

    traffic_data['DateTimeStart'] = pd.to_datetime(data.DatumFileBegin+' ' + data.TijdFileBegin)
    traffic_data['DateTimeEnd'] = pd.to_datetime(data.DatumFileEind+' ' + data.TijdFileEind)
    traffic_data['datetime_rounded'] = traffic_data.DateTimeStart.dt.round('60min')
    traffic_data['Duration'] = data.FileDuur.str.replace(',','.').astype('float64')
    traffic_data['weekday'] = traffic_data.DateTimeStart.dt.dayofweek.astype('category')
    traffic_data['month'] = traffic_data.DateTimeStart.dt.month.astype('category')

    traffic_data['HPstart'] = data.HectometerKop.str.replace(',','.').astype('float64')
    traffic_data['HPend'] = data.HectometerStaart.str.replace(',','.').astype('float64')
    traffic_data['distance'] = abs(traffic_data.HPstart-traffic_data.HPend)
    traffic_data['GemLengte'] = data.GemLengte.str.replace(',','.').astype('float64')/1000
    traffic_data[['trajectA','trajectB']] = traffic_data['traject'].str.split(' - ',n=1,expand=True)

    #Find the lat_lon_coordinates per city 
    lat_lon_df = __get_lat_lon_per_city(traffic_data)
    traffic_data['traject_city'] = traffic_data.apply(lambda x: x.trajectA if x.trajectA in lat_lon_df.city.values else x.trajectB,axis=1)
    ...
    traffic_data = pd.merge(left=traffic_data,right=weather_per_city,left_on=['traject_city','datetime_rounded'],right_on=['city','date'],how='left')

    #add categories
    traffic_data['ampm'] = traffic_data.DateTimeStart.dt.time.apply(lambda x: 'am' if x<datetime.time(ampm_border) else 'pm')
    traffic_data['shortlong'] = traffic_data.Duration.apply(lambda x: 'short' if x<shortlong_border else 'long')
    traffic_data['coldwarm'] = traffic_data.temperature_2m.apply(lambda x: 'cold' if x<10 else 'warm')
    traffic_data['rainydry'] = traffic_data.rain.apply(lambda x: 'rainy' if x>1 else 'dry')
    traffic_data['temp_cat'] = pd.qcut(traffic_data.temperature_2m,q=[0,0.2,0.4,0.6,0.8,1],labels=['freezing','cold','comfortable','warm','hot'])

    print("Data collection and cleaning done!")

    return traffic_data, lat_lon_df, weather_per_city

def update_sql_database(working_dir,data_to_update,sql_table_name):
...
def update_pickle_with_sql_database_data(working_dir,pickle_file_path,sql_table_name):
    import os
    import time
...
#%% private methods
def __get_lat_lon_per_city(traffic_data):
    cities = []
    lat = []
    lon = []
    unique_city = pd.concat([traffic_data.trajectA,traffic_data.trajectB]).unique()
